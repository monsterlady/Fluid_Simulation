...
public class FluidSimulation2D : MonoBehaviour
{
    ...
    public float userInteractionRadius;
    ...
    void Start()
    {
        Debug.Log("开始初始化FluidSimulation2D");
        ...
        // 设置计算着色器
        ComputeHelper.SetBuffer(simulationComputeShader, particlePositionsBuffer, "Positions", applyExternalForcesKernel, updateParticlePositionsKernel);
        ComputeHelper.SetBuffer(simulationComputeShader, predictedPositionsBuffer, "PredictedPositions", applyExternalForcesKernel, computeSpatialHashKernel, computeDensityKernel, computePressureKernel, computeViscosityKernel);
        ComputeHelper.SetBuffer(simulationComputeShader, hashGridIndices, "SpatialIndices", computeSpatialHashKernel, computeDensityKernel, computePressureKernel, computeViscosityKernel);
        ComputeHelper.SetBuffer(simulationComputeShader, hashGridOffsets, "SpatialOffsets", computeSpatialHashKernel, computeDensityKernel, computePressureKernel, computeViscosityKernel);
        ComputeHelper.SetBuffer(simulationComputeShader, fluidDensityBuffer, "Densities", computeDensityKernel, computePressureKernel, computeViscosityKernel);
        ComputeHelper.SetBuffer(simulationComputeShader, particleVelocitiesBuffer, "Velocities", applyExternalForcesKernel, computePressureKernel, computeViscosityKernel, updateParticlePositionsKernel);

        simulationComputeShader.SetInt("numParticles", totalParticles);

        sortingAlgorithm = new GPUSort();
    ...
    void OnDestroy()
    {
        ComputeHelper.Release(particlePositionsBuffer, predictedPositionsBuffer, particleVelocitiesBuffer, fluidDensityBuffer, hashGridIndices, hashGridOffsets);
    }
